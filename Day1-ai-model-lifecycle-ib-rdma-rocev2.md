...
[나쁜 배치 — SYS: 소켓을 가로지름]
   ...
   GPU ──PCIe── CPU0 ══UPI/xGMI══ CPU1 ──PCIe── NIC
  (소켓0)                                      (소켓1)

   ⇒ 소켓 간 링크 경유 + host memory bounce ⇒ P2P 불가/저하


[좋은 배치 — PIX: 같은 PCIe 스위치 하위]

   GPU ──┐
         ├── PCIe switch ──── (CPU 루트 컴플렉스 우회)
   NIC ──┘

   ⇒ GPUDirect RDMA 직결 ⇒ CPU·소켓 간 링크 미경유
```
