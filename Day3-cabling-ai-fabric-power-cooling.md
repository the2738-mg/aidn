...
GPU 클러스터를 실제로 짓는 물리 계층 3부작. **① 케이블을 무엇으로 어디까지 뽑을지(케이블링), ② 그 위에서 RDMA·혼잡제어·로드밸런싱·토폴로지를 어떻게 설계할지(AI 패브릭), ③ 그 랙에 전기를 어떻게 넣고 열을 어떻게 빼낼지(전원·냉각)** 를 정리한다.
...
# Part 2. 네트워크 엔지니어를 위한 딥러닝 인프라 (AI 패브릭 심화)
